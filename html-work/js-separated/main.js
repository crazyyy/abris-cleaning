$(document).ready(function () {
	...
	form.on("submit", function (e) {
		...
		var ctip = $(this).find("input[name='ctip']").val();
		...
		if (send === true) {
			$.ajax({
				...
				$('.agmodal').trigger('ag:close');
				if (ctip === 'zhaloba') {
					setTimeout(function () {$('a.zhalob-modal').trigger('click');},500);
				}else{
					setTimeout(function () {$('a.sp').trigger('click');},500);
				}
				setTimeout(function () {$('.agmodal').trigger('ag:close');},6000);
				//yaCounter25726091.reachGoal(ctip);
			});
		}
	});
...
//смена навигации
	...
	if (scroll_top > 50) {
		$('.top-panel').addClass('white-panel');
	}
	...
	$(window).scroll(function() {
		...
		if (scroll_top > 50) {
			...
			$('.top-panel').removeClass('white-panel');
		};
	});
//смена навигации
